# This file describes eFuses for ESP32E22 chip
#
...
class EspEfuses(base_fields.EspEfusesBase):
    ...
    def __init__(
        ...
        if esp.CHIP_NAME != "ESP32-E22":
            raise esptool.FatalError(
                f"Expected the 'esp' param for ESP32-E22 chip but got for '{esp.CHIP_NAME}'."
            )
        if not skip_connect:
    ...
    def postpone_efuses_from_block0_to_burn(self, block):
        """ESP32-E22 does not have SPI_BOOT_CRYPT_CNT; use only existing BLOCK0 efuses."""
        postpone_efuses = {}
        if block.id != 0:
        ...
        for efuse_name in efuses_list:
            postpone_efuses[efuse_name] = self[efuse_name].get_bitstring(
                from_read=False
            )
