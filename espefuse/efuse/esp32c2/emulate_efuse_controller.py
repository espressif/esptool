...
class EmulateEfuseController(EmulateEfuseControllerBase):
    ...
    def set_major_chip_version(self, version):
        version &= 0x3
        if version:
            self.direct_write_efuse(1, version << 20, block=2)

    def set_minor_chip_version(self, version):
        version &= 0xF
        if version:
            self.direct_write_efuse(1, version << 16, block=2)

    """ esptool method start >>"""

    def get_minor_chip_version(self):
        return (self.read_efuse(1, block=2) >> 16) & 0xF

    def get_major_chip_version(self):
        return (self.read_efuse(1, block=2) >> 20) & 0x3
