...
class EmulateEfuseController(EmulateEfuseControllerBase):
    ...
    def set_minor_chip_version(self, version):
        ...
        low = version & 0x07
        if low:
            self.direct_write_efuse(4, low << 4, block=1)
        if hi:
            self.direct_write_efuse(3, hi << 21, block=1)

    """ esptool method start >>"""

    def get_minor_chip_version(self):
        hi = (self.read_efuse(3, block=1) >> 21) & 0x01
        low = (self.read_efuse(4, block=1) >> 4) & 0x07
        return (hi << 3) + low

    def get_major_chip_version(self):
        return (self.read_efuse(3, block=1) >> 18) & 0x03

    def get_crystal_freq(self) -> int:
        return 40  # MHz (common for all chips)

    def get_security_info(self) -> dict[str, int | None]:
        return {
            "flags": 0,
            "flash_crypt_cnt": 0,
            "key_purposes": 0,
