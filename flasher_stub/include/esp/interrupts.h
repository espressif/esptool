...
{
    uint32_t old_level;
    __asm__ volatile ("rsil %0, 3" : "=a" (old_level));
    return old_level;
}
...
static inline void _xt_isr_unmask(uint32_t unmask)
{
    ...
    intenable |= unmask;
    __asm__ volatile ("wsr %0, intenable; esync" :: "a" (intenable));
}
...
{
    uint32_t interrupt;
    __asm__ volatile ("rsr %0, interrupt" : "=a" (interrupt));
    return interrupt;
}

static inline void _xt_clear_ints(uint32_t mask)
{
    __asm__ volatile ("wsr %0, intclear; esync" :: "a" (mask));
}
