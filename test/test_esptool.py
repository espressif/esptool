...
class TestGetPID:
    ...
    def _loader_for_port(self, port):
        esp = esptool.ESPLoader.__new__(esptool.ESPLoader)
        esp.cache = {"usb_pid": None}
...
class TestFlashing(EsptoolTestCase):
    ...
    def test_verify_erase_between_32M_flash(self):
        ...
        try:
            ...
            output1 = self.run_esptool(f"write_flash {BASE_OFFSET} {binary1.name}")
            assert "Hash of data verified" in output1

            output2 = self.run_esptool(f"write_flash {BASE_OFFSET} {binary2.name}")
...
class TestChipDetectionValidation(EsptoolTestCase):
    ...
    def _find_different_chip(self, chip_name, detection_method):
        ...
        for chip in esptool.CHIP_DEFS:
            if chip != chip_name and chip != "esp32s2":
                if "BETA" in chip.upper():
                    continue
                if (
                    detection_method == "chip_id"
                    and not esptool.CHIP_DEFS[chip].USES_MAGIC_VALUE
                ):
                    print(f"chip: {chip}, chip_name: {chip_name}")
                    if (
                        not hasattr(esptool.CHIP_DEFS[chip_name], "IMAGE_CHIP_ID")
                        or esptool.CHIP_DEFS[chip].IMAGE_CHIP_ID
                        != esptool.CHIP_DEFS[chip_name].IMAGE_CHIP_ID
                    ):
                        return esptool.util.expand_chip_name(chip)
                elif (
    ...
    def test_chips_with_chip_id_detection(self):
        ...
        passed_chip = esptool.util.expand_chip_name(arg_chip).upper()
        output = self.run_esptool(f"--chip {arg_chip} flash_id")
        ...
        error_output = self.run_esptool_error(f"--chip {different_chip_id} flash_id")
        assert (
            f"This chip is {passed_chip}, not {different_chip_id.upper()}."
            in error_output
        ...
        error_output = self.run_esptool_error(f"--chip {different_chip_magic} flash_id")
        assert (
            f"This chip is {passed_chip}, not {different_chip_magic.upper()}."
            in error_output
            or "Wrong chip argument?" in error_output
        )
        ...
        if arg_chip != "esp32s2":
            ...
            error_output = self.run_esptool_error("--chip esp32s2 flash_id")
            assert (
                f"This chip is {passed_chip}, not ESP32-S2." in error_output
                or "Wrong chip argument?" in error_output
            )
