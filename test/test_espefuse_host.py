#!/usr/bin/env python
#
# HOST_TEST for espefuse.py [support esp32, esp32s2, esp32s3beta2, esp32s3, esp32c3, esp32h2beta1, esp32c2]
#
# How to use it:
#
...
support_list_chips = ["esp32", "esp32s2", "esp32s3beta2", "esp32s3", "esp32c3", "esp32h2beta1", "esp32c2"]
...
class TestReadProtectionCommands(EfuseTestCase):
    ...
    def test_read_protect_efuse2(self):
        ...
        self.espefuse_py('read_protect_efuse {}'.format(efuse_name),
    ...
    def test_read_protect_efuse4(self):
        if chip_target == "esp32":
            ...
            self.espefuse_py('burn_key BLOCK_KEY0 images/efuse/128bit_key SECURE_BOOT_DIGEST')
...
@unittest.skipIf(chip_target == "esp32c2", "TODO: add support set_flash_voltage for ESP32-C2")
...
class TestBurnEfuseCommands(EfuseTestCase):
    ...
    def test_burn_mac_custom_efuse(self):
        ...
        if chip_target in ['esp32h2', 'esp32h2beta1']:
            self.espefuse_py('get_custom_mac', check_msg='aa:cd:ef:01:02:03:00:00 {}'.format(crc_msg))
        else:
            self.espefuse_py('get_custom_mac', check_msg='aa:cd:ef:01:02:03 {}'.format(crc_msg))
...
class TestBurnKeyCommands(EfuseTestCase):
    ...
    def test_burn_key_1_key_block(self):
        self.espefuse_py("burn_key -h")
        self.espefuse_py('burn_key BLOCK_KEY0 images/efuse/128bit XTS_AES_128_KEY',
                         check_msg="A fatal error occurred: Incorrect key file size 16. Key file must be 32 bytes (256 bits) of raw binary key data.",
                         ret_code=2)
        self.espefuse_py('burn_key BLOCK_KEY0 images/efuse/256bit XTS_AES_128_KEY --no-read-protect')
        output = self.espefuse_py('summary -d')
    ...
    def test_burn_key_one_key_block_with_fe_and_sb_keys(self):
        ...
        self.espefuse_py('burn_key  BLOCK_KEY0 images/efuse/256bit XTS_AES_128_KEY \
                         ...
                         check_msg="A fatal error occurred: These keypurposes are incompatible ['XTS_AES_128_KEY', 'SECURE_BOOT_DIGEST']",
                         ret_code=2)
        self.espefuse_py('burn_key  BLOCK_KEY0 images/efuse/128bit_key XTS_AES_128_KEY_DERIVED_FROM_128_EFUSE_BITS \
                                    BLOCK_KEY0 images/efuse/128bit_key SECURE_BOOT_DIGEST --no-read-protect')
        output = self.espefuse_py('summary -d')
        self.assertIn('[3 ] read_regs: 0c0d0e0f 08090a0b 04050607 00010203 03020100 07060504 0b0a0908 0f0e0d0c', output)

        self.espefuse_py('burn_key  BLOCK_KEY0 images/efuse/128bit_key XTS_AES_128_KEY_DERIVED_FROM_128_EFUSE_BITS \
                                    BLOCK_KEY0 images/efuse/128bit_key SECURE_BOOT_DIGEST')
        output = self.espefuse_py('summary -d')
        self.assertIn('[3 ] read_regs: 00000000 00000000 00000000 00000000 03020100 07060504 0b0a0908 0f0e0d0c', output)

    @unittest.skipUnless(chip_target in ["esp32s2", "esp32s3", "esp32s3beta1", "esp32c3", "esp32h2", "esp32h2beta1"], "Only chip with 6 keys")
...
class TestBurnBlockDataCommands(EfuseTestCase):
    ...
    def test_burn_block_data_with_1_key_block(self):
        ...
        self.assertIn('[2 ] read_regs: a3a2a1a0 a7a6a5a4 abaaa9a8 afaeadac b3b2b1b0 b7b6b5b4 bbbab9b8 bfbebdbc', output)
        self.assertIn('[3 ] read_regs: a3a2a1a0 a7a6a5a4 abaaa9a8 afaeadac b3b2b1b0 b7b6b5b4 bbbab9b8 bfbebdbc', output)
    ...
    def test_burn_block_data_check_errors(self):
        ...
        self.espefuse_py('burn_block_data \
                         ...
                         ret_code=2)
        self.espefuse_py('burn_block_data BLOCK0 images/efuse/192bit --offset 33',
                         check_msg="A fatal error occurred: Invalid offset: the block0 only holds",
                         ret_code=2)
        self.espefuse_py('burn_block_data BLOCK0 images/efuse/256bit --offset 4',
    ...
    def test_burn_block_data_with_offset_for_3_key_blocks(self):
        ...
        offset = 4
        self.espefuse_py('burn_block_data --offset %d BLOCK1 images/efuse/192bit_1' % (offset))
        self.check_data_block_in_log(self.espefuse_py('summary -d'), "images/efuse/192bit_1", offset=offset)

        offset = 6
        self.espefuse_py('burn_block_data --offset %d BLOCK2 images/efuse/192bit_2' % (offset))
        self.check_data_block_in_log(self.espefuse_py('summary -d'), "images/efuse/192bit_2", offset=offset)

        offset = 8
        self.espefuse_py('burn_block_data --offset %d BLOCK3 images/efuse/192bit_2' % (offset))
        self.check_data_block_in_log(self.espefuse_py('summary -d'), "images/efuse/192bit_2", offset=offset)

    @unittest.skipUnless(chip_target == "esp32c2", "The test only for esp32c2")
    def test_burn_block_data_with_offset_1_key_block(self):
        offset = 4
        self.espefuse_py('burn_block_data --offset %d BLOCK1 images/efuse/92bit' % (offset))
        output = self.espefuse_py('summary -d')
        self.assertIn('[1 ] read_regs: 00000000 03020100 00060504', output)

        offset = 6
        self.espefuse_py('burn_block_data --offset %d BLOCK2 images/efuse/192bit_1' % (offset))
        output = self.espefuse_py('summary -d')
        self.assertIn('[2 ] read_regs: 00000000 00110000 05000000 09080706 0d0c0b0a 11100f0e 15141312 00002116', output)
    ...
    def test_burn_block_data_with_offset_6_keys(self):
        offset = 4
        self.espefuse_py('burn_block_data --offset %s BLOCK_KEY0 images/efuse/192bit_1' % (offset))
        self.check_data_block_in_log(self.espefuse_py('summary -d'), "images/efuse/192bit_1", offset=offset)

        offset = 6
        self.espefuse_py('burn_block_data --offset %s BLOCK_KEY1 images/efuse/192bit_2' % (offset))
        self.check_data_block_in_log(self.espefuse_py('summary -d'), "images/efuse/192bit_2", offset=offset)

        offset = 8
        self.espefuse_py('burn_block_data --offset %s BLOCK_KEY2 images/efuse/192bit_2' % (offset))
        self.check_data_block_in_log(self.espefuse_py('summary -d'), "images/efuse/192bit_2", offset=offset)
...
@unittest.skipUnless(chip_target == "esp32", "Tests are only for esp32. (TODO: add for all chips)")
